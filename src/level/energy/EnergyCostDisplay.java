...
public class EnergyCostDisplay extends EnergyDisplay {
    ...
    public void setCost(int cost, Level level) {
        ...
        if (perTurn) {
            ...
            setText((cost < 0 ? (level.levelRenderer.energyManager.canAfford(level.getThisTeam(), -cost, false) ? NO_COLOUR : ENERGY_COST_RED) : ENERGY_COST_GREEN).display + EnergyManager.numberText(cost) + TextRenderable.ENERGY.display);
        }
    }
}
