...
public class FighterType extends UnitType {
    public static final FighterType
        ...
        w //check 2nd tutorial unit types example if interceptor class modifiers are changed
    ...
    }, FiringRenderer.THREE_UNITS, "This special variant of the " + INTERCEPTOR.getName() + " unit comes equipped with missiles that deal high damage against " + ShipClass.CRUISER.getClassName().toLowerCase() + " units. " +
    ...
    FighterType(String name, String displayName, int value, float hitPoints, float maxMovement, float maxViewRange, Action[] actions, int firingAnimFrames, float firingAnimUnitWidth, int minRange, int maxRange, float damage, Consumer<ArrayList<WeaponTemplate>> weaponGenerator, Consumer<TreeMap<UnitCharacteristic, UnitCharacteristicValue>> unitCharacteristicSetter, BiConsumer<HashMap<Action, Integer>, HashMap<Action, Integer>> actionCostSetter, Supplier<ObjPos[]> firingPositions, String description) {
