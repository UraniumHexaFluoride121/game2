...
public class StaticHitBox implements HitBox {
    ...
    public static StaticHitBox createFromOriginAndSize(float x, float y, float width, float height) {
        return new StaticHitBox(y + height, y, x, x + width);
    }
    ...
    public StaticHitBox scale(float s) {
        up *= s;
        down *= s;
        left *= s;
        right *= s;
